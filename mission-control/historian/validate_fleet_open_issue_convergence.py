...
def main():
    ...
    f=d["fleet"]
    assert (total,control,active,blocked)==(94,32,2,60)
    assert (f["open_issues"],f["control"],f["active"],f["blocked"])==(94,32,2,60)
    assert f["executable_frontier_width"]==2
    assert abs(f["control_share"]-32/94)<1e-10
    assert abs(f["active_share"]-2/94)<1e-10
    assert abs(f["blocked_share"]-60/94)<1e-10
    assert sum(len(row["active"]) for row in repos.values())==2
    assert repos["GBOGEB/cryoplant-project"]["open_issues"]==53
    assert len(repos["GBOGEB/cryoplant-project"]["blocked_breakdown"]["external_source_decision_return"])==30
    assert repos["GBOGEB/gg_MATH"]["nested_queue"]["bd_total"]==6
    print("PASS_FLEET_OPEN_ISSUE_CONVERGENCE_94_32_2_60")
