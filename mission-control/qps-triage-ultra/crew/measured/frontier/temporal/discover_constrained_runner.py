...
def write_output(enabled, labels):
...
def main():
    ...
    if declared_raw:
        try:
            ...
            if labels[0] != 'self-hosted' and 'self-hosted' not in labels:
    ...
    write_output(receipt['enabled'], labels)
    print(json.dumps({'status': receipt['status'], 'enabled': receipt['enabled'], 'declared_labels': receipt['declared_labels']}, sort_keys=True))
