...
def main() -> int:
    ...
    varying = [
        f for f in features
        if len({float(row[f]) for row in rows}) > 1
    ]
    ...
    readiness = {
        "schema": "missioncontrol.gm_i_a.multiclock_readiness.v1",
        "dataset_sha256": hashlib.sha256(raw).hexdigest() if not args.append_row else None,
        "mission": doc["mission"],
        ...
        "next_measurement_target": {
            ...
            "priority": "REPEAT_SAME_NINE_CLOCK_VECTOR_WITH_EXACT_RUN_AND_JOB_BINDING"
        },
    ...
    Path(args.pairs_out).write_text(json.dumps({
        "schema": "missioncontrol.gm_i_a.runtime_pressure_pairs.v1",
    ...
    if args.dataset_out:
        expanded = dict(doc)
        expanded["rows"] = rows
        Path(args.dataset_out).write_text(json.dumps(expanded, indent=2, sort_keys=True) + "\n", encoding="utf-8")
