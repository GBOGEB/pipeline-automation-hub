...
def main():
    ...
    require(len(args.base_sha) == 40, "base SHA must be 40 chars")
    require(actual_head == args.source_sha, f"source SHA mismatch expected={args.source_sha} actual={actual_head}")
    subprocess.check_call(["git", "cat-file", "-e", f"{args.base_sha}^{{commit}}"])
    require(
        subprocess.run(["git", "merge-base", "--is-ancestor", args.base_sha, actual_head]).returncode == 0,
        f"PR event base {args.base_sha} is not an ancestor of exact head {actual_head}",
    )

    require(manifest.get("measurement_scope") == "CAPABILITY_COVERAGE_NOT_CONCURRENCY_CAPACITY", "measurement scope must be explicit")
    require(manifest.get("concurrency_capacity_claimed") is False, "serial proof must not claim concurrency capacity")
